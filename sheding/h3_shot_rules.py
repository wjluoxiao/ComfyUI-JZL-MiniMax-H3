...
- 段首静默铁律（CRITICAL）：**每一段的开头 0.8 秒内严禁出现任何 <d> 台词**（含对白/画外音/内心独白）——[Shot 1] 起 0.8 秒只能用画面/环境声/动作铺陈，第一句台词的最早时间必须 ≥ 0.8 秒；若本段是「无限时长」的段首延续段（会在成片里被裁掉），台词必须从延续段结束之后才开始。
...
- Head-silence rule (CRITICAL): the FIRST 0.8 seconds of EVERY segment MUST contain NO <d> dialogue of any kind (spoken line / voiceover / inner monologue) — only visuals/ambience/action may occupy the first 0.8s, and the earliest dialogue timestamp must be ≥ 0.8s; if this segment starts with an infinite-length head hand-off section (which is discarded in the final cut), dialogue must begin only after that section ends.
